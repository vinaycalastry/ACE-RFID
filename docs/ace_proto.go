...
type UnwindingFilament struct {
	...
	Mode   int `json:"mode"` // 0 - Normal mode; 1 - Enhanced mode
}
...
type SlotStatus struct {
	...
	Rfid     RfidStatus `json:"rfid,omitempty"`     // 0 - No RFID information found, 1 - Unable to identify, 2 - Identified
	Source   int        `json:"source,omitempty"`   // 0 - Unknown, 1 - from RFID, 2 - User Defined, 3 - Empty
	Redirect string     `json:"redirect,omitempty"` // Redirect other consumables during automatic refill
}
...
type GetStatus struct {
	...
	ContAssistTime  float64      `json:"cont_assist_time"` // Continuous feeding time, unit: ms
